(ns looker.main
  "Kotodama WASM entrypoint for the Looker clean-room actor (L4) — Clojure port.
  ...
  facts (`looker.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "looker")
(def tier "L4")
...
(def entity-specs
  [{:entity "Look"           :plural "looks"           :id-prefix "looker_loo"
    :fields [:title :public :queryId :userId :deleted] :required [:title :public]
    :coerce {:public :bool :deleted :bool}  :refs {:queryId "Query" :userId "User"}}
   {:entity "Dashboard"      :plural "dashboards"      :id-prefix "looker_das"
    :fields [:title :folderId :userId :viewCount]     :required [:title :viewCount]
    :coerce {:viewCount :int}                :refs {:folderId "Folder" :userId "User"}}
   {:entity "Query"          :plural "queries"         :id-prefix "looker_que"
    :fields [:model :view :slug :queryTimezone :hasTableCalculations] :required [:model :view]
    :coerce {:hasTableCalculations :bool}   :refs {}}
   {:entity "User"           :plural "users"           :id-prefix "looker_use"
    :fields [:email :firstName :lastName :isDisabled]  :required [:email :firstName]
    :coerce {:isDisabled :bool}              :refs {}}
   {:entity "Folder"         :plural "folders"         :id-prefix "looker_fol"
    :fields [:name :parentId :creatorId]    :required [:name]
    :coerce {}                               :refs {}}
   {:entity "ScheduledPlan"  :plural "scheduledplans"  :id-prefix "looker_sch"
    :fields [:name :userId :enabled]        :required [:name :enabled]
    :coerce {:enabled :bool}                 :refs {:userId "User"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"looker.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "looker-compat" :tier tier :entities entities} 200])
...
(defn start! [] :looker-compat/ready)
